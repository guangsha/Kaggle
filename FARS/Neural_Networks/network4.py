...
class Network(object):
    ...
    def SGD(self, train_x, train_y, epochs, mini_batch_size, eta,
            validation_x, validation_y, test_x, test_id, validate_id, lmbda=0.0):
...
#        test_x, test_y = test_data
        ...
        validation_y_original = validation_y
...
#        self.test_mb_predictions = theano.function(
        validate_mb_predictions = theano.function(
        ...
        for epoch in xrange(epochs+1):
            for minibatch_index in xrange(num_training_batches):
                ...
                if (iteration+1) % num_training_batches == 0:
                    ...
                    Yvalidate = [validate_mb_predictions(j) for j in xrange(num_validation_batches)]
                    predict_df = pd.DataFrame(data={'ID': validate_id,'DRUNK_DR': np.ravel(Yvalidate)})
                    submit = predict_df.groupby('ID').mean()
                    correct_df = pd.DataFrame(data={'ID': validate_id,'DRUNK_DR': np.ravel(validation_y_original)})
                    correct_result = correct_df.groupby('ID').mean()
                    validation_accuracy = auc(correct_result.astype(int), submit)
            ...
            if epoch > 0 and epoch % 5 == 0:
                ...
                predict_df = pd.DataFrame(data={'ID': test_id,'DRUNK_DR': np.ravel(Ytest)})
...
#        self.y_out = T.argmax(self.output, axis=1)
    ...
    def cost(self, net):
        ...
        return -T.mean(T.log(self.output_dropout)[T.arange(net.y.shape[0]), net.y] * (net.y + 0.695))
